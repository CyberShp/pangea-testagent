...
function environmentForm(id){
 ...
 const options=new Map((state.skill_roles||[]).map(r=>[r.id,r.name?`${r.name}（${r.id}）`:r.id]));
 for(const [role] of bindings)if(!options.has(role))options.set(role,`${role}（已有角色）`);
 form(id?'编辑环境组合':'新建环境组合',input('name','环境名称',env.name)+select('policy','操作权限',[['confirm','关键操作需要确认'],['automatic','全程自动']],env.policy||'confirm')+'<label>设备角色（来自已导入的 Skill）</label><div id="role-rows"></div><p id="role-hint" class="muted"></p><button type="button" id="add-role" class="secondary">添加角色</button>',v=>{
  const roles={};
