...
const availableSkills=()=>[...state.skills,...(state.scenarios||[]).map(s=>({...s,builtin:true,mode:'structured'}))];
const skillKey=s=>JSON.stringify([s.id,s.version,!!s.builtin]);
...
function render(){const route=location.hash.slice(1)||'tasks';selected=route.startsWith('tasks/')?route.split('/')[1]:null;const section=route.split('/')[0];watchTask(selected);document.querySelectorAll('nav a').forEach(a=>a.classList.toggle('active',a.hash==='#'+section));$('title').textContent=({tasks:'任务中心',devices:'设备管理',environments:'环境组合',skills:'Skill 管理',settings:'设置与后端'})[section]||'任务中心';if(selected){renderDetail(selected).catch(error);return;}serial++;
 ...
 else if(section==='skills')$('content').innerHTML=`<div class="toolbar"><p>内置场景随平台提供；导入的 Skill 固定保存版本，更新请增加版本号。</p><div class="actions">${btn('编写助手','authoring','','secondary')}${btn('导入文件夹','import-folder','','secondary')}${btn('导入 ZIP','import-skill')}</div></div>`+table(['能力','版本','校验模式','操作'],availableSkills().map(s=>`<tr><td><strong>${esc(s.name)}</strong>${s.builtin?' <span class="badge">内置</span>':''}<div>${esc(s.id)}</div></td><td>${esc(s.version)}</td><td>${s.mode==='structured'?'符合框架约定':'普通 Skill'}</td><td class="actions">${btn('查看','skill-view',skillKey(s),'secondary')}${link('导出',s.builtin?`/api/scenarios/export?id=${encodeURIComponent(s.id)}`:`/api/skills/export?id=${encodeURIComponent(s.id)}&version=${encodeURIComponent(s.version)}`)}${s.builtin?'<span class="muted">随平台更新</span>':btn('删除','delete-skill',JSON.stringify([s.id,s.version]),'quiet')}</td></tr>`))+`<details class="card footer-card"><summary>框架验收样例</summary><p>只在本机模拟诊断交互，不连接任何真实设备。</p>${btn('导入模拟样例','example','','secondary')}</details>`;
...
$('content').addEventListener('click',event=>{const element=event.target.closest('[data-action]');if(!element)return;const action=element.dataset.action,id=element.dataset.id;const dispatch=async()=>{
 ...
 if(action==='skill-view'){const [sid,version,builtin]=JSON.parse(id);const pkg=await request(builtin?'/api/scenarios/detail?id='+encodeURIComponent(sid):'/api/skills/detail?id='+encodeURIComponent(sid)+'&version='+encodeURIComponent(version));form(pkg.name,select('skillfile','包内文件',Object.keys(pkg.files).map(n=>[n,n]))+'<pre id="skill-text"></pre>',null);const show=()=>$('skill-text').textContent=pkg.files[$('f-skillfile').value];$('f-skillfile').onchange=show;show();return;}
